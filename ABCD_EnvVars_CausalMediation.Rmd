...
```{r}
# culture 
(rel_culture_corr <- cor.test(rel1_culture_pccontrib$Contribution, rel2_culture_pccontrib$Contribution, method = "spearman"))
...
#neighborhood
(rel_neighborhood_corr <- cor.test(rel1_neighborhood_pccontrib$Contribution, rel2_neighborhood_pccontrib$Contribution, method = "spearman"))
...
fullvar_corr <- full_sample_data %>%
  dplyr::select(c(3:55, 65:67)) %>%
  cor()


#diag(variable_corr) <-  0

fullvar_corr_pvals <- full_sample_data[,c(3:55, 65:67)] %>%
        cor.mtest(method = "spearman")

png("fullvar_corrplot.png", res = 400, width = 14000, height = 14000)
variable_corrplot <- corrplot(fullvar_corr, method = 'color', type = "full", diag = FALSE, p.mat =fullvar_corr_pvals$p, 
                              insig = "label_sig", cl.pos = "b", cl.cex = 4, pch.col = "black", pch.cex = 3, tl.cex = 2.5, tl.srt = 30, tl.col = "black",
                              col = colorRampPalette(c("red", "white","blue"))(200))
dev.off()
#tl.cex = .5, tl.srt = 25, tl.col = "black"  tl.pos = "n",
...
```{r}
secondrel3_culture_totaleffect <- lm(Neurocog_BPPC3_LearnMem ~ sociodemo_PC1, data = second_release_data)
summary(secondrel3_culture_totaleffect)

secondrel3_culture_demenviro_patha <- lm(culture_PC1 ~ sociodemo_PC1, data = second_release_data)
summary(secondrel3_culture_demenviro_patha)

secondrel3_culture_envirocog_pathb <- lm(Neurocog_BPPC3_LearnMem ~ sociodemo_PC1 + culture_PC1, data = second_release_data)
summary(secondrel3_culture_envirocog_pathb)

firstrel3_culture_mediation <- mediate(secondrel3_culture_demenviro_patha, secondrel3_culture_envirocog_pathb, treat = 'sociodemo_PC1', mediator = 'culture_PC1',
                             boot = TRUE, sims = 5000, boot.ci.type = 'bca')
